...
coloredlogs.install(fmt="%(asctime)s,%(msecs)03d [%(levelname)s] %(message)s", level='SUCCESS')
verboselogs.install()
...
def replace_all_text_in_slides(presentation_id, replacements_dict):
	token_path = 'token.json'
	creds = Credentials.from_authorized_user_file(token_path)
...
def update_textbox_backgrounds(presentation_id, substrings_colors_dict):
    token_path = 'token.json'
    creds = None

...
parser.add_argument('-l', '--level', type=str, choices=['CRITICAL', 'ERROR', 'WARNING', 'INFO', 'DEBUG',
                                                        'NOTICE', 'SUCCESS', 'SPAM', 'VERBOSE', 'NOTSET'],
...
if __name__ == "__main__":
    ...
    parser.add_argument("-log", required=False,
                        choices=['CRITICAL', 'ERROR', 'SUCCESS', 'WARNING', 'NOTICE', 'INFO', 'VERBOSE', 'DEBUG',
                                 'SPAM'], default='SUCCESS')
    args = parser.parse_args()
    level = args.log
...
for a in range(2):
    ...
    for t in range(3):
        ...
        REPLACE_WORDS[f"{{{k}{t + 1}_prev_ccwm}}"] = tba.get_team_stats(tba.prev_comp(comp_code, team, l), team, l)[
            "ccwm"]
        REPLACE_WORDS[f"{{{k}{t + 1}_prev_opr}}"] = tba.get_team_stats(tba.prev_comp(comp_code, team, l), team, l)[
            "opr"]
        REPLACE_WORDS[f"{{{k}{t + 1}_prev_dpr}}"] = tba.get_team_stats(tba.prev_comp(comp_code, team, l), team, l)[
            "dpr"]
        ...
        REPLACE_WORDS[f"{{{k}{t + 1}_prev_status}}"] = remove_substrings(
            i[tba.prev_comp(comp_code, team, l)]["overall_status_str"]) if tba.prev_comp(comp_code, team,
                                                                                         l) != None else ""
...
for a in range(2):
    for t in range(3):
        ...
        color = parse_rank_and_assign_color(remove_substrings(
            remove_substrings(i[tba.prev_comp(comp_code, team, l)]["overall_status_str"]) if tba.prev_comp(comp_code,
                                                                                                           team,
                                                                                                           l) != None else ""))
